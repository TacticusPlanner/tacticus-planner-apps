...
for (const abilityId of [...referencedAbilityIds].sort()) {
  raidBossAbilities[abilityId] =
    abilityById.get(abilityId)?.text?.name ?? formatWords(abilityId)
}

const raidBossTraits = {}
for (const traitId of [...referencedTraitIds].sort()) {
  raidBossTraits[traitId] = traitById.get(traitId)?.name ?? formatWords(traitId)
}
