...
/**
 * The one-line "Farming strategy" summary shown in the read-only detail view: nothing for a Level goal
 * (uncosted), the chosen strategy for a Rank goal, and otherwise whether the farm locations are an
...
export function goalFarmingSummary(
  ...
  kind: { isLevel: boolean; isRank: boolean }
): string | null {
  if (!detail || kind.isLevel) return null
  if (kind.isRank) {
