...
The page SHALL offer a name search plus faction, alliance, attack-type, game-mode, damage-type and trait filters. The name search SHALL remain visible at all times and SHALL NOT be counted as a filter; the remaining controls MAY be collapsed behind a toggle that reports how many are active. Faction, alliance, attack type and game mode SHALL be single-select; damage type and trait SHALL be multi-select, and each multi-select SHALL offer its own clear control that resets only that control. Filter options SHALL be derived from the listed catalog, and an option whose id is blank SHALL NOT be offered. Search SHALL match the localized NPC name case-insensitively. A variation **matches** when its `factionId` equals the selected faction (if any), when its melee, ranged, active-ability, or passive-ability damage-type ids include every selected damage type (if any), when its trait ids include every selected trait (if any), and when the group name matches the search (if any). A group SHALL be listed when at least one of its available variations matches. While filters are active, the Variation selector for the selected group SHALL offer only the matching variations; if the currently selected variation stops matching, the page SHALL select the first matching variation in the group. Clearing all filters SHALL restore every listed NPC and every available variation. If the selected group has no matching variation, the page SHALL keep the selected group's URL and show the no-matching-variation state instead of navigating away. Filter state SHALL be page-local (not in the URL) and SHALL reset when the user leaves the NPC Library.
