...
- [x] 1.1 Confirm the companion `tacticus-planner-api` `add-raid-boss-portraits`
...
- [x] 2.1 Add `packages/game-catalog/src/game-entities/raid-boss-portrait-overrides.ts`
...
- [x] 2.2 Add `packages/game-catalog/src/game-entities/icons/raid-boss.ts`:
...
- [x] 2.3 Export the helpers from the package barrel; verify an import from the
      web app resolves under `pnpm typecheck`.

## 3. Assets

- [x] 3.1 Copy every `snowprint_assets/characters/*.png` referenced by the
...
- [x] 4.1 Add `questUnitId?: string` to the game-catalog raid-boss record type
...
- [x] 5.1 In `entities/raid-boss/ui/raid-boss-portrait.tsx`, resolve `src` via
...
- [x] 5.2 In `raid-boss-detail.tsx`, render an icon (via `fieldNpcIcon`) beside
...
- [x] 5.3 Update `raid-bosses-page.tsx` to pass each field npc's `questUnitId`
      (from its unit-set record) into the icon resolver.

## 6. Verification + gates

- [x] 6.1 Manual browser check on `/library/raid-bosses` (list + several boss
...
- [x] 6.2 Run `pnpm test:run`, `pnpm typecheck`, `pnpm lint`, `pnpm lint:fsd`,
