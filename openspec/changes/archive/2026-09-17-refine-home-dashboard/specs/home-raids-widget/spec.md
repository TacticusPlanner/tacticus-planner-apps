...
- **GIVEN** a battle location is part of today's real schedule with 3 planned raids remaining
- **WHEN** the widget renders that location
- **THEN** it shows one row with the location's compact chip, "3×", and its primary reward icon — no unit portrait or goal label
...
- **GIVEN** Goal A plans 2 raids at node "Indomitus Elite 3" and Goal B plans 3 raids at the same node, and no daily cap at that node is exceeded by the combined total
- **WHEN** the widget renders
- **THEN** "Indomitus Elite 3" appears once, showing "5×", instead of appearing once under each goal as Today does
