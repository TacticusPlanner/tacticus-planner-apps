...
- [x] 4.3 Manually verified on the running dev server: project detail shows Project/Filter/Group By labeled together in the header, Group offers only "No grouping"/"By type" (no "By unit"), and Goals Overview's own Group control still offers "By unit" unchanged. Found and fixed a real bug in the process: the mobile icon-only Project switcher didn't open at all (Radix's item-aligned content positioning needs `SelectValue` to align against, which the icon-only trigger doesn't have — content rendered thousands of pixels off-screen while the page stayed locked). Fixed in `project-select.tsx` with `position="popper"` (same fix `goal-filters.tsx` already carries), with a regression test added.
