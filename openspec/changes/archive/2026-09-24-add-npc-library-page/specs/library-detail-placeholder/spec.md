...
**Reason**: The requirement covered both Machines of War and NPCs; NPCs now have a real detail view (see the `npc-library` capability), so the NPC placeholder scenario is obsolete and the requirement is re-stated for Machines of War only.

**Migration**: `/library/npcs` renders the NPC Library page. Machines of War keep the placeholder under the requirement added below; no consumer change is needed.
