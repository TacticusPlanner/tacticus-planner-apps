...
- `GoalsLayout` (`pages/goals/ui/goals-board/goals-layout.tsx`) wraps Overview/Projects/Insights and owns the Planning Settings button + dialog state. Overview and Insights stay inside `pages/goals`; Projects moves out (see Decision 6).
...
**6. Projects becomes its own top-level page slice, `pages/projects`, instead of living inside `pages/goals`.** Routes are already composed at the `app` layer today — `app/routes.tsx` imports each top-level section's `routes` export (`goalsRoutes`, `dailiesRoutes`, etc.) rather than one page slice importing another, so promoting Projects to a sibling slice follows an existing, precedented pattern rather than introducing a new one. The one genuinely new wrinkle: Projects keeps its current URL and tab position, nested under `/goals/project` inside `GoalsLayout`, so `app/routes.tsx` now composes `GoalsLayout` (from `pages/goals`) as the parent element around Projects' route element (from `pages/projects`) for that one nested route — no other section in this app nests two different page slices' routes together today, so this specific composition is new even though slice-per-route itself isn't. Verify `pnpm lint:fsd` accepts this composition once implemented.

**7. Projects' management surface: `ProjectToolbar` is retired; `ManageProjectsSheet` is narrowed, not replaced.** The Sheet's embedded project list is removed (the list now lives permanently on the page instead), leaving the Sheet as a create/edit form only, opened either blank ("New project") or pre-filled (a row's "Edit" action). The new on-page list (in `pages/projects`, or a `features/project-management` component it composes) renders every project — including archived — as a row with inline lifecycle actions: Edit (opens the narrowed Sheet), Set active, Archive/Restore. The Sheet keeps the existing `status` mapping (Archive → `"Archived"`, Restore → `"Active"`) and the existing Archive guard (`disabled` when pending, default, or the active plan) unchanged from today's `ManageProjectsSheet`. Bulk "Pause all"/"Resume all" are removed entirely — redundant with each goal's own row-level pause/resume.
...
- **Nesting `pages/projects`' route inside `pages/goals`' `GoalsLayout` at the `app/routes.tsx` layer is a new composition shape for this codebase** → Mitigation: it's the same "app composes top-level page slices" pattern already used everywhere else, applied one level deeper for this one nested route; run `pnpm lint:fsd` and the full route tree in-browser before considering Decision 6 done.
