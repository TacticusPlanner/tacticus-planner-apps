...
- Projects becomes a dedicated project-management surface: the `ProjectToolbar` + `ManageProjectsSheet` (a Sheet-based side panel) are replaced by an inline "Manage Projects" list rendered directly on the page, with per-project inline actions (Edit, Set active, Archive/Restore) and an inline "+ New project" form. Selecting a project row replaces the standalone project Select as how the user picks which project's goals show below. **BREAKING**: the bulk "Pause all"/"Resume all" project actions are removed — redundant with the pause/resume already available on each goal row.
- Projects gains the same Type/Sort/Group filters Overview already has, scoped to the selected project's goals.
...
- `project-management`: Defines Projects as the dedicated surface for creating, editing, activating, and archiving projects inline on the page (no side-panel Sheet), including the project list, its per-row actions, and the project-scoped goal list's filters.
...
- Affected files: `apps/web/src/fsd/pages/goals/ui/goals-board/goals-layout.tsx`, `goals-page.tsx`, `goals-list.tsx`; `apps/web/src/fsd/pages/goals/ui/projects/projects-page.tsx`; `apps/web/src/fsd/pages/goals/ui/insights/insights-page.tsx`; `apps/web/src/fsd/pages/dailies/ui/raids-layout.tsx`, `raids-plan-page.tsx`; `apps/web/src/fsd/features/project-management/ui/project-toolbar.tsx` and `manage-projects-sheet.tsx` (retired/replaced); `apps/web/src/fsd/entities/project/ui/project-select.tsx` (adopted more broadly).
- New shared UI: a status-select component (Unfulfilled/Reached/Archived with a reached-indicator) usable by both Overview and Projects; an inline project-row management component replacing the Sheet.
- i18n: new/changed strings across the `goals` and `dailies` locale namespaces (status select labels, inline project form, removed pause/resume copy) in `apps/web/public/locales`.
- Joyride tutorials: `today.tutorial.tsx` and `raids-plan.tutorial.tsx` need selector updates for any moved `data-testid`s. Overview and Projects currently have no tutorial coverage at all; since both get materially changed page flows here, this change adds tutorials for them per repository convention.
