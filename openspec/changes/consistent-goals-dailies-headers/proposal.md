...
- Affected files/slices: `apps/web/src/fsd/pages/goals/ui/goals-board/goals-layout.tsx`, `goals-page.tsx`, `goals-list.tsx`; `apps/web/src/fsd/pages/goals/ui/insights/insights-page.tsx`; `apps/web/src/fsd/pages/dailies/ui/raids-layout.tsx`, `raids-plan-page.tsx`; `apps/web/src/fsd/features/project-management/ui/project-toolbar.tsx` (retired) and `manage-projects-sheet.tsx` (narrowed to a form-only Sheet); `apps/web/src/fsd/entities/project/ui/project-select.tsx` (adopted more broadly); `apps/web/src/fsd/entities/goal/` (new: shared status-select and goal filters). Projects moves out of `pages/goals` into its own new top-level page slice, `apps/web/src/fsd/pages/projects/`, composed into the existing `/goals/project` route alongside `GoalsLayout` at the `app/routes.tsx` layer.
