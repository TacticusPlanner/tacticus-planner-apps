...
**Reason**: Superseded by "Desktop sidebar exposes each section's child pages via a hover/click flyout" below — the sidebar now deliberately renders a chevron and a flyout of child pages for any section that has them, which this requirement previously forbade outright.

**Migration**: No consumer-facing migration is needed. Internally, any code or test asserting that the desktop sidebar renders no child links, chevron, or sub-list for a section must be updated to expect the new flyout affordance instead.

### Requirement: Desktop header title stays fixed to the top-level section, with a child-picker beside it

**Reason**: Superseded by "Desktop header shows a static section/child breadcrumb, with no picker" below. Child-page navigation no longer lives in the header at all (see the sidebar flyout requirement), so the header no longer needs an interactive picker beside a fixed title — it becomes a plain, non-interactive breadcrumb that may itself show the active child's label.

**Migration**: No consumer-facing migration is needed. Internally, any code or test asserting an inline nav-link row or breadcrumb dropdown in the desktop header must be updated to expect the new static breadcrumb, and any test relying on selecting a child via that header dropdown must instead use the sidebar flyout.
