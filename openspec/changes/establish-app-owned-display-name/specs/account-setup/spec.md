...
- **WHEN** a signed-in user with a configured Tacticus API key and confirmed display name opens a protected route
...
#### Scenario: Key exists but name is unconfirmed

- **WHEN** a signed-in user with a configured key but unconfirmed display name opens a protected route
- **THEN** they are taken to the name-confirmation step and the route's own content is not shown
