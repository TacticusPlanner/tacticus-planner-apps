...
**Reason**: Its "Final archived membership remains protected" scenario is contradicted by this change. A goal whose only membership is an archived project can now be removed from it, because the Default project — which can never be archived and always exists — is available as a relocation destination. Keeping the requirement would leave two rules in this capability disagreeing about the same removal.

**Migration**: Replaced by "Archived memberships remain visible and relocatable" below, which keeps the archived-visibility behavior unchanged and restates the final-membership case as relocation. No stored data or API behavior changes; only the last-membership outcome for an archived-only goal differs, and it differs in the same way it does for any other project.
