...
This mirrors the backend's existing denormalization pattern (raw collections → served, self-contained datasets) rather than introducing a new architecture. `eventDefinitions`/`eventOccurrences` are authored by hand, same as every other catalog source file; `eventsCalendar` is computed at build time — date-indexed, with every entry self-contained (no client-side join needed against `eventDefinitions` to render a calendar cell).
