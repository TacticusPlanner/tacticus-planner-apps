...
- [ ] 1.1 Consume the API's normalized Rank target/conflict data and show exact-target validation in goal creation; verify distinct Silver3/Gold1 acceptance and same-target 409 draft-preservation tests.
- [ ] 1.2 Update Goals/project rows and status/delete actions for independent Rank ids and target labels; verify desktop/mobile tests including delete/recreate and completed history.
...
- [ ] 2.3 Update project membership/assembly/relocation eligibility to compare normalized Rank targets while preserving non-Rank rules and server-authoritative conflicts; verify multi-select, stale-save, and atomic rejection tests.
...
- [ ] 3.4 Run `pnpm test:run`, `pnpm typecheck`, `pnpm lint`, `pnpm lint:fsd`, and `git diff --check`; verify all gates pass.
