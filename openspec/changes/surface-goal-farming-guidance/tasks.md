...
- [ ] 1.1 Trace existing goal need, eligible-node, blocker, and XP-book allocation outputs for a populated Rank/Level project; verify which fields are already available client-side and whether a same-named API companion is required (it is for the rarity field: author and apply the API contract first).
...
- [ ] 1.3 Add `xpBookRarity` (Common–Mythic, default Legendary) to planning settings: types, default, API round-trip, dialog control, and consumption by the guidance and `computeLevelGoalCost`; verify unit and dialog tests, including fallback to Legendary for a missing or unsupported value.
